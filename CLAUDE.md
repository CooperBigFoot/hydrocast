...
model-training — a uv workspace containing three packages: coach (training harness + CLI), data-leak (data loading), and time-flies (model architectures).

## Workspace Workflow

- `uv sync` from workspace root installs all packages
- `uv run coach <cmd>` for CLI
- `uv run pytest` runs all tests across all packages
- `uv run ruff check .` lints everything
- Per-package version bump: `cd <package> && uv run bump-my-version bump patch`
- Submodule workflow: commit inside the submodule first, then in workspace root update the submodule ref with `git add <submodule> && git commit`
...
1. `uv run bump-my-version bump patch` — modifies `pyproject.toml` and `src/coach/__init__.py`
